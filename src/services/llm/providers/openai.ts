...
type CommitMessageStructuredOutput = {
...
const commitMessageSchema = {
    ...
    properties: {
        commit_message: {
            type: 'string',
            description: 'Conventional Commit formatted message ready for git commit.',
        },
        git_command: {
            type: 'string',
            description: 'Complete git commit command that uses the commit_message.',
        },
    },
    required: ['commit_message', 'git_command'],
    additionalProperties: false,
};

function parseCommitMessage(content: string): CommitMessageStructuredOutput {
    const parsed = JSON.parse(content);
    if (!parsed || typeof parsed.commit_message !== 'string' || typeof parsed.git_command !== 'string') {
        throw new Error('Invalid commit message response payload.');
    }
    return parsed;
}

const commitMessageStructuredTextFormat = makeParseableTextFormat<CommitMessageStructuredOutput>({
    type: 'json_schema',
    name: 'commit_message_payload',
    description: 'Commit message and git command pair generated by Git Commit Genie.',
    strict: true,
...
export class OpenAIService extends BaseLLMService {
    ...
    private shouldFallbackStructuredOutputs(err: any): boolean {
        const status = err?.status ?? err?.response?.status;
        if (status && status !== 400) { return false; }
        const msg = String(err?.message || err?.error?.message || '').toLowerCase();
        if (!msg) { return false; }
        return msg.includes('json_schema') ||
            msg.includes('response_format') ||
            msg.includes('structured outputs') ||
            msg.includes('strict outputs are not available') ||
            msg.includes('text format for this model is not supported');
    }
    ...
    async generateCommitMessage(diffs: DiffData[], options?: { token?: vscode.CancellationToken }): Promise<LLMResponse | LLMError> {
        ...
        try {
            ...
            let resp: any;
            let parsed: CommitMessageStructuredOutput | null = null;
            let useStructuredOutputs = true;
            let structuredOutputsSucceeded = false;

            for (let attempt = 0; attempt < 4; attempt++) {
                try {
                    if (useStructuredOutputs) {
                        resp = await this.openai!.responses.parse({
                            model,
                            input: [
                        ...
                        parsed = (resp as any)?.output_parsed ?? null;
                        structuredOutputsSucceeded = true;
                    } else {
                        ...
                        const text = this.getResponseOutputText(resp);
                        parsed = text ? this.safeExtractJson<CommitMessageStructuredOutput>(text) : null;
                    }
                    break;
                } catch (e: any) {
                    const code = e?.status || e?.code;
            ...
            if (parsed?.commit_message) {
                return { content: parsed.commit_message };
            }

            const refusal = this.extractRefusal(resp);
            if (refusal) {
                return { message: refusal, statusCode: 400 };
            }

            const fallbackText = this.getResponseOutputText(resp);
            if (fallbackText) {
                const jsonResponse = this.safeExtractJson<CommitMessageStructuredOutput>(fallbackText);
