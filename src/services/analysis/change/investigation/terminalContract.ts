...
export function buildInvestigationProtocolLines(finishToolName: string): string[] {
    return [
        ...
        'Work through plan targets in their listed priority order. Resolve the uncertainty that can most change the factual commit message first.',
        'For an exact changed symbol, start with its definition or changed file. Trace references, callers, callees, implementations, configuration, or tests only when that relation answers a planned question.',
        'For a file, hunk, relation, config key, CLI flag, or API route, search the exact changed name or path and then read the narrow source region that establishes its consumer or behavior.',
        'Use the captured after snapshot by default. Read before only when the diff does not already establish the previous behavior and the comparison matters to the claim.',
        'A text-search hit is a candidate, not proof of symbol resolution or a complete call graph. Read the relevant source when surrounding control flow matters.',
        'An empty search is not proof that a caller, consumer, or test does not exist. Refine the exact name/path once when useful, otherwise leave the question unresolved.',
        'Do not repeat the same tool call, tour unrelated directories, or collect repository background that cannot change the commit message.',
        'If the plan contains targets, collect at least one real E* item before finishing. listDirectory, searchRepositoryMemory, and searchCode with searchType "name" are navigation-only and do not themselves publish E* evidence.',
        `Call ${finishToolName} as soon as the material planned questions are answered or are clearly unanswerable with the remaining tools.`,
        'There is no reward for spending the whole tool budget, and an unnecessary lookup only adds noise or weakens evidence precision.',
        'Anything you cannot confirm stays an unresolved question. Do not replace it with a plausible guess or infer intent from co-occurrence.',
        'README and other prose are not primary evidence; use them only for terminology, architecture, and capability naming.',
        'Successful evidence-producing reads and searches publish E* repository evidence ids; empty, failed, or navigation-only results publish none. Diff hunks already carry D* ids.',
        'Reading a test establishes its asserted contract, not that the test passed.',
