...
export function buildTerminalContractLines(): string[] {
    return [
        ...
        '- supported_inference: a conclusion that needs D* and E* evidence read together. Requires at least one id.',
...
export function buildCorrectionLines(failure: AgentTerminalFailure, finishToolName: string): string[] {
    ...
    return [
        ...
        ...formatFieldIssuesForModel(failure.fieldIssues).map(line => `- ${line}`),
