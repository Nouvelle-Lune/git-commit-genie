...
Consolidation selects a seed investigation and at most 19 related investigations using paths and BM25 over recorded questions and observations. Inputs retain questions, tool reasons and arguments, order, results, claims and excerpts. Eligible episodes must contain a non-Memory investigation observation before they can fit into the input budget. Existing experiences are updated through explicit handles and retain their identity; a no-findings response does not delete previous experience. Failed entries cannot replace existing history.
...
The experience format intentionally replaces the unreleased old handbook and episode formats. The current Episode protocol is version 3 and the manifest protocol is version 4. Explicitly clear repository memory before using it; there is no data migration or automatic startup reset. Clearing preserves the existing accounting boundary and does not delete unrelated logs.
